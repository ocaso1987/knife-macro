...
impl MacroTrait for EnumNameDerive {
    fn load(
        ...
        context.insert_bool("crate_dryrun", false);
        let enum_name = input.item_enum.as_ref().unwrap().ident.to_string();
        context.insert_string("enum_name", enum_name.to_string());
            ...
            .enumerate()
        {
            ...
            let key = if field_len > 0 {
                ...
                format!("{}::{}({})", enum_name.to_string(), name, place_str)
            } else {
                format!("{}::{}", enum_name.to_string(), name,)
            };
        ...
        context.insert_value("enum_variants", Value::Array(enum_variant_array));
    }
    ...
    fn process(&self, context: &mut HashMap<String, ContextType>, _input: &mut InputInfo) {
        context.insert_template(
            ...
            "#,
            vec!["enum_name", "generics_quote", "enum_variants"].map(|x| x.to_string()),
        );
    }
}
